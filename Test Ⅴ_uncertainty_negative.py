...
gdp_path = r'D:\source\percaptiaGDPnew.csv'
f_GDP_Per = pd.read_csv(gdp_path, encoding = 'ANSI')

fpop_folder_path = 'D:/source/Future POP_shi'
fGDP_folder_path = 'D:/source/Future GDP_shi'

out_path = r'D:\source\6Test4allfactors_negative'

vSSPs=['SSP2']#,'SSP5']
vYears=[x for x in range(2017, 2021)]
iters = 5000
...
for _ssp in vSSPs:
    for _year in vYears:
        ...
        for idx in range(iters):
            ...
            _T  = random.choice(model)
            print(_year)
            print(_T)
            a = random.uniform(0.5055, 0.6285)
            b = random.uniform(-0.1499, 0.09555)
            c = random.uniform(0.90, 0.93)

            result = pd.DataFrame()
            result = pd.merge(f_temp, f_POP, on = 'ID_new')
            result=pd.merge(result, f_GDP_Per, on = '市')

            result['defT_new1'] = (a * np.log10(result['Area'])+b)*c

            
            result['daily_intensity'] =  result[_T] + result['defT_new1'] - result['MMT']-0.49-0.24 * result['建筑比例'] #- result['def_MMT']
            ...
            result['defT_new1'] = (a * np.log10(result['Area'])+b)*c # 计算热岛强度的升温
            ...
            for i in range(len(coef_list)):       
                ...
                #     print(row.Temp) # 输出每一行
                    ...
                    T = getattr(row,str(_T))+ getattr(row,coef_list[i]) * row.defT_new1/row.sum_coef_area - row.MMT-0.49-0.24 * row.建筑比例 #- row.def_MMT
                    coef = row.RR_change
            ...
            model_list.append(_T)
        # 创建一个包含两个列表数据的DataFrame
        data_frame = pd.DataFrame({'pop': sum_pop_list, 'ED': sum_ED_list,'model': model_list})
